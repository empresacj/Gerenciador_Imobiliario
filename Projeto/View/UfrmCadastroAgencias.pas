...
begin
  cedNumero.EditValue := ObjAgencias.CodigoAgencia;
  ...
  if ObjBanco.Find(Format('IDBANCO = %d', [ObjAgencias.CodigoBanco])) then
   begin
    ...
    btnCadastrarBanco.Enabled := False;
   end;
...
begin
  if ObjAgencias.New then
   begin
    ObjAgencias.CodigoAgencia   := cedNumero.EditValue;
   ...
   on E: Exception do
    begin
     if (Pos('VIOLATION', UpperCase(E.Message)) > 0) then
	    TDialogFunctions.Atencao('Já existe uma agência cadastrada com os dados informados.');
    end;
  end;
  StayOpen := not Result;
end;
