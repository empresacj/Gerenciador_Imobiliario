...
procedure TfrmCadastroCidades.Destruicao;
begin
  inherited;
  FObjEstado.Free;
end;
...
begin
  ...
  try
   ...
   TDBFunctions.GetClientDataSet(ObjQry, cdsEstado);
   FObjEstado := TEstado.NewInstanceFromDataSet(ObjQry, dmComuns.SQLConnection) as TEstado;
  finally
   ObjQry.Free;
  end;
end;
...
begin
  ...
  if ObjEstado.Find(Format('IDESTADO = %d', [ObjCidade.CodigoEstado])) then
   lkeEstado.EditValue := ObjEstado.CodigoEstado;
...
begin
  ...
  ObjCidade.CodigoEstado := ObjEstado.CodigoEstado;
