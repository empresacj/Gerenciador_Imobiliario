...
type
  ...
  published
    ...
    class function GetSequence(const Generator: String): Integer;
  end;

implementation

uses
  SysUtils, Provider, UdmComuns;
...
class function TDBFunctions.GetSequence(const Generator: String): Integer;
var
  ObjQry: TSQLQuery;
begin
  ObjQry := Self.SQLDataSet(TSQLQuery, dmComuns.SQLConnection) as TSQLQuery;

  try
   ObjQry.SQL.Add(Format('SELECT GEN_ID(%s, 1) AS VALOR FROM RDB$DATABASE', [Generator]));
   ObjQry.Open;

   Result := ObjQry.FieldByName('VALOR').AsInteger;
  finally
   ObjQry.Free;
  end;
