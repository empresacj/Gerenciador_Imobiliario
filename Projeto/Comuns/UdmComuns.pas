...
type
  ...
  public
    ...
    function ExecForm(InstanceClass: TComponentClass; var Reference; const bCliente: Boolean = False): Boolean;
...
function TdmComuns.ExecForm(InstanceClass: TComponentClass;
  var Reference; const bCliente: Boolean): Boolean;
begin
  Result := False;

  if bCliente then
   if not SelecionaCliente then Exit;
...
function SelecionaCliente: Boolean;
begin
  Result := (ipCodigoCliente > 0) or TfrmSelecionarCliente.DialogSelect(ipCodigoCliente);
end;
